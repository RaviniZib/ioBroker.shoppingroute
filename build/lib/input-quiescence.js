...
exports.createInputQuiescenceSeries = createInputQuiescenceSeries;
exports.collectExternalChange = collectExternalChange;
exports.deferAfterActiveSort = deferAfterActiveSort;
exports.recordSortListRun = recordSortListRun;
exports.isInputQuiet = isInputQuiet;
exports.activeValueSignature = activeValueSignature;
function createInputQuiescenceSeries(startedAt) {
    return {
        startedAt,
        lastExternalAt: 0,
 ...
 */
function collectExternalChange(current, signature, observedAt, quietMs) {
    if (current?.lastExternalSignature === signature)
        return { series: current, collected: false };
 ...
 */
function deferAfterActiveSort(current, observedAt, quietMs) {
    return {
        ...current,
        quietUntil: Math.max(current.quietUntil, observedAt + quietMs),
    };
}
function recordSortListRun(current) {
    return { ...current, sortListRuns: current.sortListRuns + 1 };
}
function isInputQuiet(current, observedAt) {
...
//# sourceMappingURL=input-quiescence.js.map