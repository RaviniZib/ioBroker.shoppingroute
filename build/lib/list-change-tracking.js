...
exports.classifyHeaderBatchObservation = classifyHeaderBatchObservation;
...
function sameExpectedValues(current, expected, transition) {
    ...
    const observedCreates = new Map();
    for (const [id, expectedValue] of expected) {
        if (deletedIds.has(id) && !current.has(id))
            continue;
        if (current.get(id) !== expectedValue)
            return false;
    }
    for (const [id, value] of current) {
        ...
        if (count > (allowedCreates.get(value) || 0))
            return false;
 ...
 */
function classifyHeaderBatchObservation(items, expected, transition) {
    const current = activeListValues(items);
    if (!sameExpectedValues(current, expected, transition))
        return 'ambiguous';
    const allIds = new Set(items.map(item => scalarText(item.id)).filter(Boolean));
    if (transition.deletes.some(id => allIds.has(id)))
        return 'pending';
    const observedCreates = valueCounts([...current]
        .filter(([id]) => !expected.has(id))
        .map(([, value]) => value));
    const expectedCreates = valueCounts(transition.creates);
    for (const [value, count] of expectedCreates) {
        if ((observedCreates.get(value) || 0) !== count)
            return 'pending';
    }
    return 'confirmed';
}
