...
import type { AlexaWriteReadinessSnapshot } from './alexa-write-readiness';
import type { ConfirmationResult } from './confirmation-wait';

export type RecoveryLateSettlementObservation = ConfirmationResult | 'pending';

export interface RecoveryLateSettlementWaitOptions {
    from: string;
    to: string;
    baseline: AlexaWriteSnapshot;
    timeoutMs: number;
    pollIntervalMs: number;
    probe: () => Promise<AlexaWriteReadinessSnapshot>;
...
function sameMetadata(
    left: AlexaWriteEvidence,
    right: AlexaWriteEvidence,
): boolean {
...
function evidenceAdvanced(previous: AlexaWriteEvidence, current: AlexaWriteEvidence): boolean {
...
function evidenceUnchanged(previous: AlexaWriteEvidence, current: AlexaWriteEvidence): boolean {
    const previousVersion = numericValue(previous.version);
    const currentVersion = numericValue(current.version);
    const previousUpdated = numericValue(previous.updatedDateTime);
    const currentUpdated = numericValue(current.updatedDateTime);
...
function acknowledgedRemoteMetadata(evidence: AlexaWriteEvidence): boolean {
 ...
 */
export function classifyRecoveryLateSettlement(
    from: string,
    to: string,
    baseline: AlexaWriteSnapshot,
    current: AlexaWriteReadinessSnapshot,
): RecoveryLateSettlementObservation {
    const jsonValue = current.json.value;
    const itemValue = current.item.value;
    const allowed = new Set([from, to]);

    if (
        (jsonValue !== undefined && !allowed.has(jsonValue)) ||
        (itemValue !== undefined && !allowed.has(itemValue))
    ) return 'ambiguous';
    if (jsonValue === undefined || itemValue === undefined) return 'pending';
    ...
    if (jsonValue === from && itemValue === from) {
        if (jsonAdvanced || itemAdvanced) return 'ambiguous';
        if (
            current.json.acknowledged === true &&
            current.item.acknowledged === true &&
            acknowledgedRemoteMetadata(current.item) &&
    ...
    if (fromAdvanced) return 'ambiguous';
    return 'pending';
}
 ...
 */
export async function observeRecoveryLateSettlement(
    options: RecoveryLateSettlementWaitOptions,
): Promise<ConfirmationResult> {
    const timeoutMs = Math.max(0, options.timeoutMs);
    const pollIntervalMs = Math.max(1, options.pollIntervalMs);
    let elapsedMs = 0;
    let latest: RecoveryLateSettlementObservation = 'pending';

    while (true) {
        if (options.shouldAbort?.()) return 'ambiguous';
        latest = classifyRecoveryLateSettlement(
            options.from,
            options.to,
            options.baseline,
            await options.probe(),
        );
        if (latest === 'confirmed' || latest === 'ambiguous') return latest;
        if (elapsedMs >= timeoutMs) return latest === 'not-applied' ? 'not-applied' : 'ambiguous';
