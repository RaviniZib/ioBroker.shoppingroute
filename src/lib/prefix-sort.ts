...
export interface PrefixSortPlan {
    desired: PrefixTarget[];
    updates: PrefixUpdate[];
    deletes: PrefixDelete[];
    creates: PrefixCreate[];
    fallback: boolean;
...
export function parseSortPrefix(text: string): SortPrefix | undefined {
    ...
    if (!Number.isInteger(number) || number < 0 || number > 99 || !originalText) return undefined;
...
function categoryOrder(routes: RouteConfig[], marketName: string, category: string): number {
    ...
    const route = routes.find(entry =>
        normalize(entry.market) === normalize(marketName) && normalize(entry.category) === normalize(category),
    );
    return Number.isFinite(Number(route?.order)) ? Number(route?.order) : 9999;
}

function active(items: AlexaListItem[]): AlexaListItem[] {
    return items.filter(item => item && item.completed === false && item.id && stripSortPrefix(item.value));
}

function usableVersion(item: AlexaListItem): number | undefined {
...
export function buildPrefixTargets(
    ...
    for (const item of current) {
        ...
        if (headerMarket) {
            ...
            if (market) {
                ...
                entries.push(item);
                headersByMarket.set(normalize(market.name), entries);
            }
            continue;
        }
    ...
    for (const entry of assigned) {
        ...
        if (
            ...
            const existing = headersByMarket.get(normalize(market))?.shift();
            const text = formatMarketHeader(market);
            targets.push({
                key: existing ? `id:${existing.id}` : `header:${normalize(market)}`,
                id: existing?.id,
    ...
    if (targets.length > 99) throw new Error(`Die Liste enthält ${targets.length} Soll-Einträge; maximal 99 sind zulässig.`);
    return targets;
}

function distribute(lowerExclusive: number, upperExclusive: number, count: number): number[] | undefined {
    const available = upperExclusive - lowerExclusive - 1;
    if (count < 1) return [];
    if (available < count) return undefined;
    const numbers: number[] = [];
    for (let index = 1; index <= count; index++) {
        const number = Math.floor(lowerExclusive + (index * (upperExclusive - lowerExclusive)) / (count + 1));
        if (number <= lowerExclusive || number >= upperExclusive || numbers.includes(number)) return undefined;
        numbers.push(number);
    }
    return numbers;
}

function staleActiveItems(items: AlexaListItem[], desiredIds: Set<string>): AlexaListItem[] {
    return active(items).filter(item => !desiredIds.has(String(item.id)));
}

function requireWritableItem(item: Pick<PrefixTarget, 'id' | 'version'>, action: string): asserts item is { id: string; version: number } {
    if (!item.id || !item.version) throw new Error(`${action}: Item-ID oder positive Amazon-Version fehlt.`);
}
...
function buildFallback(
    ...
    for (const item of staleActiveItems(items, new Set(desired.slice(0, rebuildFrom).flatMap(target => target.id ? [target.id] : [])))) {
        ...
        if (!version) throw new Error(`DELETE für ID ${item.id}: positive Amazon-Version fehlt.`);
 ...
 */
export function createPrefixSortPlan(items: AlexaListItem[], desired: PrefixTarget[]): PrefixSortPlan {
    if (desired.length > 99) throw new Error('Maximal 99 aktive Listeneinträge sind zulässig.');
    const fixed: Array<{ index: number; number: number }> = [];
    for (let index = 0; index < desired.length; index++) {
        const target = desired[index];
        if (target.id && target.currentPrefix !== undefined) fixed.push({ index, number: target.currentPrefix });
    }
    ...
    for (const boundary of boundaries) {
        ...
        if (!numbers) return buildFallback(items, desired, previousIndex + 1);
        numbers.forEach((number, offset) => assigned.set(previousIndex + 1 + offset, number));
        if (boundary.index < desired.length) assigned.set(boundary.index, boundary.number);
        previousIndex = boundary.index;
        previousNumber = boundary.number;
    }

    const updates: PrefixUpdate[] = [];
    const creates: PrefixCreate[] = [];
    for (let index = 0; index < desired.length; index++) {
        const target = desired[index];
        ...
        if (target.currentValue === to) continue;
...
export function expectedValues(plan: PrefixSortPlan): string[] {
    if (plan.fallback) {
...
export function verifyPrefixResult(
    items: AlexaListItem[],
    plan: PrefixSortPlan,
    allowAdditionalExternalItems = false,
): PrefixVerification {
    const current = active(items);
    const byId = new Map(current.map(item => [String(item.id), String(item.value).trim()]));
    for (const deletion of plan.deletes) {
        if (byId.has(deletion.id)) return { ok: false, reason: `Gelöschte ID ${deletion.id} ist weiterhin aktiv.` };
    }
    const updated = new Map(plan.updates.map(update => [update.id, update.to]));
    ...
    if (allowAdditionalExternalItems) {
        ...
        for (const create of plan.creates) createdCounts.set(create.value, (createdCounts.get(create.value) || 0) + 1);
    ...
    if (actual.length !== expected.length) return { ok: false, reason: `Erwartet ${expected.length}, gefunden ${actual.length} aktive Items.` };
